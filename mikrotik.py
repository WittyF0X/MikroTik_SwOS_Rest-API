...
def get_Mikrotik_Vlan(url, username, password, port_number=None, port_name=None, vlan_id=None, output_only=None):
    ...
    while port_instance < total_ports:
        vlan = convert_Mikrotik_Vlan_Receive(hex_string=vlans['vlni'][port_instance])
        if (vlan == 'Only Untagged'):
            vlan = int(vlans['dvid'][port_instance], 16)
        object = {
            'ports': str(port_instance + 1),
            'description': str(links[port_instance]['port_name']),
            'vlan': str(vlan),
            'swstatus': str(links[port_instance]['link_active']),
        }
        port_instance += 1
        output.append(object)
...
def get_Mikrotik_Vlan_Config(url, username, password, port_number):
    query = 'vlan.b'
    method = 'GET'
    response = send_Mikrotik_Rest_Method(url=url, query=query, method=method, username=username, password=password)
    vlans = convert_Mikrotik_Json(input_object=response)
    if 'mbr' in vlans[0]:
        ...
        for vlan in vlans:
            object = {
                ...
                'status': convert_Mikrotik_Hex_Array(hex_string=vlans[vlan_instance]['mbr'], pad_length=26)
            }
            vlan_instance += 1
            if(object['status'][port_number]):
                output.append(object)
    else:
        vlan_instance = 0
        output = []
        for vlan in vlans:
            object = {
                "vlan_id": int(vlans[vlan_instance]['vid'], 16),
                'status': [('0x01' in i or '0x02' in i) for i in vlans[vlan_instance]['prt']]
            }
            vlan_instance += 1
            if(object['status'][port_number-1]):
                output.append(object)
...
def set_Mikrotik_Switch_Port(url, username, password, port_number, port_name=None, enabled=True, auto_neg=True):
    ...
    for speed_hex in mikrotik_config['spdc']:
        if speed_count <= (len(mikrotik_config['spdc']) - 1):
            ...
            new_mikrotik_config += f"'{speed_hex}'"
    if len(mikrotik_config['nm']) > 6:
        fct = f"fctc:{mikrotik_config['fctc']},fctr:{mikrotik_config['fctr']}"
    else:
        fct = f"fct:{mikrotik_config['fct']}"
    new_mikrotik_config += f"],dpxc:{mikrotik_config['dpxc']},{fct}}}"
...
def set_Mikrotik_Trank(url, username, password, port_number, vlan_id=None):
    set_Mikrotik_Vlan(url, username, password, port_number, vlan_receive='Only Tagged', vlan_id=vlan_id)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
                    prog='SSHpy',
                    description='SSH connect python',
                    epilog='Hello my dear friend')
    
    parser.add_argument('--host', metavar='host', type=str,
                    help='host of machine')
    parser.add_argument('--vlan', metavar='vlan', type=str,
                    help='vlan for port')
    parser.add_argument('--port', metavar='port', type=int,
                    help='port of machine', default=None)
    parser.add_argument('--username', metavar='username', type=str,
                    help='username of machine', default='admin')
    parser.add_argument('--password', metavar='password', type=str,
                    help='password of machine', default='NETSsw1214')
    parser.add_argument('--command', metavar='command', type=str,
                    help='command to machine')
    parser.add_argument('--newname', metavar='newname', type=str,
                    help='new name for port')
    ...
    dd = {
        'show'      : lambda: get_Mikrotik_Vlan('http://' + args['host'], args['username'], args['password']),
        'report'    : lambda: set_Mikrotik_Switch_Port('http://' + args['host'], args['username'], args['password'], port_number=args['port'], port_name=args['newname']),
        'trunk'     : lambda: set_Mikrotik_Trank('http://' + args['host'], args['username'], args['password'], port_number=args['port'], vlan_id=args["vlan"]),
        'access'    : lambda: set_Mikrotik_Vlan('http://' + args['host'], args['username'], args['password'], port_number=args['port'], vlan_id=args["vlan"]),
        'trport'    : lambda: get_Mikrotik_Vlan_Config('http://' + args['host'], args['username'], args['password'], port_number=args['port'])
    }
    print(dd[args['command']]())
    # except Exception as e:
    #     print(f'Something went wrong! type -h\nerr: {e}')